...
const ContactsView = ({ navigation, serverUrl }) => {
    ...
    // Use the useFocusEffect hook to reload users when the screen gains focus
    useFocusEffect(
        React.useCallback(() => {
            loadUsers();
        }, [])
    ...
    const groupedUsers = filteredUsers.reduce((acc, user) => {
        const [lastName] = user.lastName.split(' ').reverse();
        ...
        <SafeAreaView style={{ backgroundColor: '#f2f2f2' }}>
            <ScrollView contentContainerStyle={styles.container}>
                ...
                {loading ? (
                    ...
                    Object.entries(groupedUsers).map(([letter, sectionUsers]) => (
                                ...
                                <>
                                    ...
                                    <View style={styles.sectionItems}>
                                        {sectionUsers.map((user) => (
                                            <View key={user.contactId} style={styles.cardWrapper}>
                                                <TouchableOpacity
                                                    onPress={() => navigation.navigate('ClientDetails', { paramKey: user.contactId })}
                                                >
                                                    <View style={styles.card}>
                                                        {user.image ? (
                                                            <Image alt="" resizeMode="cover" source={{ uri: user.image }} style={styles.cardImg} />
                                                        ) : (
                                                            <View style={[styles.cardImg, styles.cardAvatar]}>
                                                                <Text style={styles.cardAvatarText}>{user.lastName[0]}</Text>
                                                            </View>
                                                        )}
