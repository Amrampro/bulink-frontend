...
export default function EditMyCompany({route, navigation}) {

    const serverUrl = `${API.organizationReadAll}/${route.params.paramKey}`;
    ...
    useEffect(() => {
        const getCompany = async () => {
            try {
                ...
                // console.log(companyInfo.type); // This can work
                // console.log(companyInfo['type']); // This second mathod too can work
                setName(companyInfo.name)
                setSlogan(companyInfo.slogan)
                setType(companyInfo.type)

                // console.log(companyInfo.first_name)
            } catch (error) {
                Alert.alert('Error', 'Failed to fetch company! Please try again. ' + error.message);
            } finally {
                // If the load exists
                // setLoading(false);
            }
    ...
    const handleUpdateCompany = async () => {
        try {
            ...
            const response = await axios.put(serverUrl, updateCompany)
        ...
        <SafeAreaView style={{backgroundColor: '#f2f2f2'}}>
            <ScrollView contentContainerStyle={styles.container}>
                <View style={styles.formContainer}>
                    ...
                    <TouchableOpacity style={styles.addButton} onPress={handleUpdateCompany}>
